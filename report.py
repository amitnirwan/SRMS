...
#title
        title=Label(self.root,text="View Students Results", font=("goudy old style", 20,"bold"),bg="#033054",fg="white").place(x=10,y=15,width=1180,height=50)
        ...
        try:
            if self.var_search.get()=="":
                ...
                if row!=None:
                    ...
                    self.course.config(text=row[4])
                    self.marks.config(text=row[5])
                    self.full.config(text=row[6])
                    self.per.config(text=row[7])
                ...
                if row==None:
                   messagebox.showerror("Error","Invalid student result ",parent=self.root)
                    ...
                    if op==True:
                        ...
                        con.commit()
                        messagebox.showinfo("Delete", "Result deleted suceesfully",parent=self.root)
                        self.clear()
